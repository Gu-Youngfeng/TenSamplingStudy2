#ifndef ENABLE_HUSH_INTERACTIVE
...
#endif
#ifndef ENABLE_FEATURE_EDITING
	do {
		G.flag_SIGINT = 0;
		fputs(prompt_str, stdout);
#ifdef ENABLE_FEATURE_EDITING_SAVEHISTORY
		fflush_all();
#endif
...
#include "math.h"
#include "match.h"
#ifdef ENABLE_HUSH_RANDOM_SUPPORT
# include "random.h"
#else
...
#if defined(ENABLE_HUSH_IF) || defined(ENABLE_HUSH_LOOPS) || defined(ENABLE_HUSH_CASE)
...
static const char hush_version_str[] = "HUSH_VERSION=";
...
enum {
	...
	RES_DONE  ,
#endif
#if defined(ENABLE_HUSH_LOOPS) || defined(ENABLE_HUSH_CASE)
	RES_IN    ,
#endif
#ifdef ENABLE_HUSH_CASE
	RES_CASE  ,
 ...
 * xtrace          off
 */
static const char o_opt_strings[] = "pipefail\0";
enum {
	OPT_O_PIPEFAIL,
	NUM_OPT_O
};
 ...
 */
#if !defined(BB_MMU) && defined(ENABLE_HUSH_LOCAL)
/* all params are used */
#elif defined(BB_MMU) && defined(ENABLE_HUSH_LOCAL)
...
static void  set_local_var_from_halves(const char *name, const char *val)
{
...
#ifdef ENABLE_HUSH_BRACE_EXPANSION
# define MAYBE_BRACES "{}"
#else
# define MAYBE_BRACES ""
#endif
 ...
 */
#if defined(DEBUG_EXPAND) || defined(DEBUG_GLOB)
...
#if defined(ENABLE_HUSH_TICK) || defined(ENABLE_SH_MATH_SUPPORT) || defined(ENABLE_HUSH_DOLLAR_OPS)
...
#if defined(ENABLE_SH_MATH_SUPPORT) || defined(ENABLE_HUSH_TICK)
	case '(': {
		unsigned pos;

		ch = i_getch(input);
		nommu_addchr(as_string, ch);
# ifdef ENABLE_SH_MATH_SUPPORT
		if (i_peek(input) == '(') {
			ch = i_getch(input);
			nommu_addchr(as_string, ch);
			o_addchr(dest, SPECIAL_VAR_SYMBOL);
...
static int encode_string(o_string *as_string,
		...
		int process_bkslash)
{
#ifndef ENABLE_HUSH_BASH_COMPAT
 ...
 */
static  const char *expand_one_var(char **to_be_freed_pp, char *arg, char **pp)
{
	const char *val = NULL;
...
#if defined(ENABLE_HUSH_BASH_COMPAT) && defined(ENABLE_SH_MATH_SUPPORT)
...
#if defined(ENABLE_FEATURE_SH_STANDALONE) || defined(BB_MMU)
...
#ifdef ENABLE_FEATURE_SH_STANDALONE
...
#if defined(ENABLE_HUSH_LOOPS) || defined(ENABLE_HUSH_FUNCTIONS)
