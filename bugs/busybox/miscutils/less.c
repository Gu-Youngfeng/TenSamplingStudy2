...
#ifdef ENABLE_FEATURE_LESS_BRACKETS
...
static void keypress_process(int keypress)
{
	...
	case 'q': case 'Q':
		less_exit(EXIT_SUCCESS);
		break;
#ifdef ENABLE_FEATURE_LESS_MARKS
	case 'm':
		add_mark();
		buffer_print();
		break;
	...
	case 'E':
		examine_file();
		break;
#ifdef ENABLE_FEATURE_LESS_FLAGS
	case '=':
		m_status_print();
		break;
#endif
#ifdef ENABLE_FEATURE_LESS_REGEXP
	case '/':
		option_mask32 &= ~LESS_STATE_MATCH_BACKWARDS;
		regex_process();
		break;
	...
	case '?':
		...
		break;
#endif
#ifdef ENABLE_FEATURE_LESS_DASHCMD
	case '-':
		flag_change();
		buffer_print();
		break;
...
#ifdef ENABLE_FEATURE_LESS_WINCH
static void sigwinch_handler(int sig)
{
#ifdef ENABLE_FEATURE_LESS_WINCH
#ifdef ENABLE_FEATURE_LESS_ASK_TERMINAL
	winch_counter++;
#endif
#endif
}
