...
#if defined (ENABLE_FEATURE_SUID) && defined (ENABLE_FEATURE_WGET_AUTHENTICATION)
static void
handle_mkd(void)
{
...
{
	int retval;
#if defined (ENABLE_SU) && defined (ENABLE_HWCLOCK) && defined (ENABLE_LOGIN)
...
#if BB_MMU
	opts = getopt32(argv,   "vSt:T:", &G.timeout, &abs_timeout, &G.verbose, &verbose_S);
#else
	opts = getopt32(argv, "l1vSt:T:", &G.timeout, &abs_timeout, &G.verbose, &verbose_S);
	if (opts & (OPT_l|OPT_1)) {
