...
#include "libbb.h"
#ifdef ENABLE_FEATURE_HTTPD_USE_SENDFILE
...
#ifdef ENABLE_FEATURE_HTTPD_RANGES
	off_t range_start;
	off_t range_end;
	off_t range_len;
#endif
...
#ifdef ENABLE_FEATURE_HTTPD_CONFIG_WITH_SCRIPT_INTERPR
...
#define hdr_buf bb_common_bufsiz1
	char *hdr_ptr;
	int hdr_cnt;
#ifdef ENABLE_FEATURE_HTTPD_ERROR_PAGES
	const char *http_error_page[ARRAY_SIZE(http_response_type)];
#endif
#ifdef ENABLE_FEATURE_HTTPD_PROXY
...
static void free_Htaccess_list(Htaccess **pptr)
{
	free_llist((has_next_ptr**)pptr);
}

static void free_Htaccess_IP_list(Htaccess_IP **pptr)
{
	free_llist((has_next_ptr**)pptr);
}
...
#ifdef ENABLE_FEATURE_HTTPD_AUTH_MD5
			char *md5_passwd;
