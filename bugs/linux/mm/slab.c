...
static int  noaliencache_setup(char *s)
{
	use_alien_caches = 0;
	return 1;
}
...
static int  slab_max_order_setup(char *str)
{
...
static int  slab_memory_callback()
{
 ...
 */
static void  set_up_node(struct kmem_cache *cachep, int index)
{
	int node;
...
static  void alloc()
{
	return cache_alloc(cachep, flags);
}
...
#ifdef CONFIG_TRACING
void kmem_cache_alloc_node_trace()
{
	void *ret;
...
static  void node(size_t size, gfp_t flags, int node, unsigned long caller)
{
	struct kmem_cache *cachep;
...
#if defined(DEBUG_SLAB) || defined(CONFIG_TRACING)
void node(size_t size, gfp_t flags, int node)
{
	return __do_kmalloc_node(size, flags);
}


void node_track_caller(size_t size, gfp_t flags,
...
#endif /* DEBUG_SLAB || CONFIG_TRACING */
#endif /* CONFIG_NUMA */
 ...
 */
static  void *__do_kmalloc(size_t size, gfp_t flags,
					  unsigned long caller)
{
	struct kmem_cache *cachep;
	void *ret;
...
static int  slab_proc_init(void)
{
#ifdef DEBUG_SLAB_LEAK
	proc_create(proc_slabstats_operations);
#endif
	return 0;
}
