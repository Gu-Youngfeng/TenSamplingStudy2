...
static void mem_limit_func(void)
{
	char *cp, *end;
	unsigned long limit;

	/* We need this before __setup() functions are called */
	#ifdef PROC_PAGE_MONITOR
	limit = MAX_MEM;
...
{
	unsigned long init_begin = (unsigned long)_begin;
	unsigned long init_end = (unsigned long)_end;
...
void mem_init(void)
{
	...
	initsize = (unsigned long)_end - (unsigned long)_begin;
	...
	reservedpages = 0;
{
	...
	printk("virtual kernel memory layout:\n"
	       ...
	       _begin, _end,
	       ((unsigned long)_end - (unsigned long)_begin) >> 10,
