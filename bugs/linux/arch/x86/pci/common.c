...
int pci_routeirq;
int noioapicquirk;
#ifdef X86_REROUTE_FOR_BROKEN_BOOT_IRQS
...
static int can_skip_ioresource_align(const struct dmi_system_id *d)
{
	...
	printk("PCI: %s detected, can skip ISA alignment\n", d->ident);
	return 0;
}
...
static int  set_bf_sort(const struct dmi_system_id *d)
{
	if (pci_bf_sort == pci_bf_sort_default) {
		...
		printk( "PCI: %s detected, enabling pci=bfsort.\n", d->ident);
	}
	return 0;
}
...
static int  assign_all_busses(const struct dmi_system_id *d)
{
	...
	printk( "%s detected: enabling PCI bus# renumbering"
			" (pci=assign-busses)\n", d->ident);
	return 0;
}
#endif

static const struct dmi_system_id const pciprobe_dmi_table[] = {
#ifdef X86_32
/*
...
struct pci_bus *  pcibios_scan_root(int busnum)
{
	...
	if (!sd) {
		printk("PCI: OOM, not probing PCI bus %02x\n", busnum);
		return NULL;
	}
...
struct pci_bus *  pci_scan_bus_on_node(int busno, struct pci_ops *ops, int node)
{
	...
	sd = kzalloc(sizeof(*sd), GFP_KERNEL);
	if (!sd) {
		printk("PCI: OOM, skipping PCI bus %02x\n", busno);
		return NULL;
	}
	sd->node = node;
